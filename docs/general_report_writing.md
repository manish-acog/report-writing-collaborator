...
- **ADK orchestration driver** (extends `src/report_writing_collaborator/agent/`) — for each
...
This puts the ADK-specific orchestration loop in `src/report_writing_collaborator/agent/`
...
- **Options:** A — put schema-building and rendering inside
  `src/report_writing_collaborator/agent/`, coupled to ADK types. B (chosen) — keep them in
