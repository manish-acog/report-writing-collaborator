...
2. **Mark every factual claim.** In a `found` field's `value`, place a
   ...
   array.
3. **A table field has no in-cell markers.** A `table`-typed field's
...
4. **Keep markers and citations paired.** Every marker must point to a real
...
5. **Keep multiple markers separate.** When one claim needs more than one
...
6. **Cite pages precisely.** Use one `Citation` per page and place their
   complete markers next to each other. Never put a page range into one
   citation.
7. **Inspect images only when needed.** Use `inspect_image` only when a field's
   evidence actually depends on what it shows — most fields won't need it.
8. **Write clearly and efficiently, not tersely.** One point per sentence, no
...
9. **Synthesize, don't reproduce.** Write findings in your own words rather
